...
final class MovieScreen: UIView {
    ...
    func createButtons() {
        for button in buttons {
            let button = createCustomButton(withImage: UIImage(named: "rate") ?? UIImage(), withText: button)
            buttonsStackView.addArrangedSubview(button)
        }
    }
    
     func createCustomButton(withImage image: UIImage, withText text: String) -> UIButton {
        let button = UIButton()
